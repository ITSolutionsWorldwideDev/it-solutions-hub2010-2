...
export function AnimatedGlobe() {
  const [readyToRenderGlobe, setReadyToRenderGlobe] = useState(false);

  useEffect(() => {
    const raf1 = requestAnimationFrame(() => {
      requestAnimationFrame(() => setReadyToRenderGlobe(true));
    });
  ...
  const sampleArcs = [
    ...
    { order: 3, startLat: 51.5072, startLng: -0.1276, endLat: 40.7128, endLng: -74.006, arcAlt: 0.3, color: colors[2] },
    { order: 4, startLat: 34.0522, startLng: -118.2437, endLat: 22.3193, endLng: 114.1694, arcAlt: 0.3, color: colors[0] },
  ];

  return (
    <div className="flex flex-col items-center justify-center py-20 h-[90vh] md:h-[45rem] bg-[#175864] relative w-full overflow-hidden">
      <div className="w-full relative h-full px-4 flex flex-col items-center">
        <motion.div
        ...
        <div className="absolute inset-0 w-full h-full flex items-center justify-center z-10 overflow-hidden">
          <div className="w-full h-full [&>canvas]:w-full [&>canvas]:h-full">
            {readyToRenderGlobe && <World data={sampleArcs} globeConfig={globeConfig} />}
          </div>
        </div>
      </div>
    </div>
