...
export default async function CreativeTeamSection({ locale }: { locale: string }) {
  ...
  const team = [
    {
      ...
      image: "/assets/images/profile/Sheetal_Devi1.webp",
    ...
    },
    {
      ...
      image: "/assets/images/profile/ZEb_Raja1.webp",
    ...
    },
    {
      ...
      image: "/assets/images/profile/Ahmed1.webp",
  ...
  return (
    <div className="w-full max-w-7xl relative text-center text-black font-lexend mx-auto my-12 py-3 md:py-4 lg:py-12 px-4 md:px-4 lg:px-5">
      ...
      {/* Yahan gap-x-2 ko mazeed kam karke gap-x-1 ya gap-x-0 kiya gaya hai */}
      <section className="flex flex-wrap gap-x-4 gap-y-6 justify-center items-center py-6 w-full">
        {team.map((member) => (
          <div key={member.name} className="flex justify-center w-full sm:w-[calc(50%-8px)] lg:w-[calc(33.333%-16px)]">
            <TeamMember {...member} />
          </div>
        ))}
      </section>
